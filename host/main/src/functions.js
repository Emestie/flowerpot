...
export const showNotification = (level, data) => {
...
export function buildIconPath(level, hasChanges, hiRez) {
    if (hasChanges)
        level = level + "d";
...
export function iconUpdateTask(level, hasChanges) {
    ...
    try {
        ...
        if (process.platform === "win32") {
            ...
            if (level !== 4)
                getAppWindow()?.setOverlayIcon(nidot, "dot");
            else
                getAppWindow()?.setOverlayIcon(null, "no-dot");
        }
    }
    catch (ex) { }
}
